defmodule Ledger do
  ...
  defp date(%Date{year: year, month: month, day: day}, :en_US), 
    do: "#{format_day_or_month(month)}/#{format_day_or_month(day)}/#{format_year(year)}"
  defp date(%Date{year: year, month: month, day: day}, :nl_NL), 
    do: "#{format_day_or_month(day)}-#{format_day_or_month(month)}-#{format_year(year)}"
  ...
  # Note: there is a problem when currency has 3 or more decimal places, since decimal separator preceeds thousands separator
  # we'll leave a guard here to note others that this doesn't support such cases
  defp whole(cents, decimal_places, options) when decimal_places <= 2 do
    thousand_separator = Keyword.fetch!(options, :thousand_separator)
    without_cents = div(cents, 10 ** decimal_places) |> abs()
