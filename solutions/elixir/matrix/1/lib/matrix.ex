defmodule Matrix do
  ...
  def from_string(input, rows \\ [])
  def from_string(<<_, _::binary>> = input, []), do: from_string(input, [[]])
  def from_string(<<?\s, rest::binary>>, rows), do: from_string(rest, rows)
  def from_string(<<?\n, rest::binary>>, [row | rows]), do: from_string(rest, [[], Enum.reverse(row) | rows])
  def from_string(<<_, _::binary>> = input, [row | rows]) do
    case Integer.parse(input) do
      {n, rest} -> from_string(rest, [[n | row] | rows])
      :error -> raise "Invalid input"
    end
  end
  def from_string(<<>>, []), do: %Matrix{rows: [], columns: []}
  def from_string(<<>>, [row | rows]), 
