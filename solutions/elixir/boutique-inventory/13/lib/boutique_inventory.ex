defmodule BoutiqueInventory do
  ...
  def sort_by_price(inventory), 
    do: Enum.sort_by(inventory, fn item -> item.price end)
  ...
  def update_names(inventory, old_word, new_word), 
    do: Enum.map(inventory, fn item -> %{item | name: String.replace(item.name, old_word, new_word)} end)
  ...
  def total_quantity(item), 
    do: Map.values(item[:quantity_by_size]) |> Enum.reduce(0, fn a, b -> a + b end)
end