defmodule ScaleGenerator do
  ...
  def step([tonic, tone | _ ], tonic, "m"), do: tone
  def step([tonic, _, tone | _ ], tonic, "M"), do: tone
  def step([tonic, _, _, tone | _ ], tonic, "A"), do: tone
  def step([_ | scale], tonic, step), do: step(scale, tonic, step)
  ...
  def chromatic_scale(tonic \\ "C") do
    gen_scale(~w(A A# B C C# D D# E F F# G G#), tonic)
  end
  ...
  def flat_chromatic_scale(tonic \\ "C") do
    gen_scale(~w(A Bb B C Db D Eb E F Gb G Ab), tonic)
  end

  defp gen_scale(scale, tonic) do
    Stream.cycle(scale)
    |> Stream.drop(Enum.find_index(scale, &(&1 == String.capitalize(tonic))))
    |> then(&Enum.take(&1, 12) ++ [String.capitalize(tonic)])
  end
  ...
  def scale(tonic, pattern) do
    [tone | _] = basic_scale = find_chromatic_scale(tonic)
    scale(pattern, basic_scale, [tone])
  end

  defp scale(<<interval, pattern::binary>>, basic_scale, [tone | _] = scale),
    do: scale(pattern, basic_scale, [step(basic_scale, tone, <<interval>>) | scale])
  defp scale(<<>>, _, scale), do: Enum.reverse(scale)
end
