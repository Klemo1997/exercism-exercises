defmodule NewPassport do
  ...
  def get_new_passport(now, birthday, form) do
    with {:ok, timestamp} <- enter_building(now),
      {:ok, manual} <- timestamp |> DateTime.from_unix!() |> DateTime.to_naive() |> find_counter_information(),
      ...
      {:coffee_break, message} -> {:retry, NaiveDateTime.add(now, @retry_minutes, :minute)}
      err -> err
    end
  end
