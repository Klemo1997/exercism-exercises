defmodule Say do
  ...
  defp groups(number, groups \\ [])
  defp groups(0, groups), do: groups
  defp groups(number, groups) when number >= 1_000_000_000, 
    do: groups(rem(number, 1_000_000_000), [numeric_word(div(number, 1_000_000_000)) <> " billion" | groups])
  defp groups(number, groups) when number >= 1_000_000,
    do: groups(rem(number, 1_000_000), [numeric_word(div(number, 1_000_000)) <> " million" | groups])
  defp groups(number, groups) when number >= 1_000,
    do: groups(rem(number, 1_000), [numeric_word(div(number, 1_000)) <> " thousand" | groups])
  defp groups(number, groups), do: [numeric_word(number) | groups] |> Enum.reverse()
