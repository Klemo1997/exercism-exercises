defmodule RelativeDistance do
  ...
  defp graph(family_tree),
    do: Enum.reduce(family_tree, :digraph.new(), fn {parent, siblings}, graph -> connect_all(graph, [parent | siblings]) end)

  defp connect_all(graph, people),
    do: tap(graph, &for(person_a <- people, person_b <- people, person_a < person_b, do: connect(&1, person_a, person_b)))
